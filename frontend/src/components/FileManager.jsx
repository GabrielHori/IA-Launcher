...
const FileManager = ({ language }) => {
  ...
  const [isDeleting, setIsDeleting] = useState(false); // État de suppression en cours
  ...
  const fetchModels = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Gestion sécurité : data peut être un tableau direct ou un objet avec .models
        const modelsList = Array.isArray(data) ? data : (data.models || []);
        
        const formattedModels = modelsList.map(m => ({
          ...m, 
          // Si la taille n'est pas en GB, on essaie de la convertir
          size_gb: m.size_gb || (m.size ? (m.size / (1024 ** 3)).toFixed(2) : 'N/A')
        }));
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:11451/api/v1/models/${encodeURIComponent(modelName)}`, {
        method: 'DELETE'
      });
      
      if (res.ok) {
        // Suppression réussie : on retire de la liste locale immédiatement
        setInstalledModels(prev => prev.filter(m => m.name !== modelName));
        // Et on rafraîchit tout au cas où
        await fetchModels();
      } else {
        alert("Erreur : Le modèle n'a pas pu être supprimé.");
      }
    } catch (err) {
    ...
    <div className="p-8 w-full h-full overflow-y-auto animate-fade-in custom-scrollbar">
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <div className="grid grid-cols-2 gap-4 w-full">
                <button 
                    ...
                    className={`py-4 rounded-2xl border font-black uppercase text-[10px] tracking-widest ${isDarkMode ? 'border-white/10 hover:bg-white/10' : 'border-black/10 hover:bg-black/5'}`}
                >
                    Annuler
                </button>
                <button 
                    onClick={confirmDelete} 
                    className="py-4 rounded-2xl bg-red-600 hover:bg-red-500 text-white font-black uppercase text-[10px] tracking-widest shadow-lg shadow-red-500/20 disabled:opacity-50"
                    disabled={isDeleting}
                >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
        ...
        <div className={`p-6 rounded-[32px] border backdrop-blur-xl flex items-center gap-8 ${isDarkMode ? 'bg-white/[0.03] border-white/10 shadow-2xl' : 'bg-black/[0.02] border-black/5 shadow-xl'}`}>
            ...
            <div className="p-5 bg-indigo-600 text-white rounded-[24px] shadow-lg shadow-indigo-500/30">
               <Database size={32} />
            </div>
        </div>
      </div>
      ...
      <div className={`flex items-center gap-4 border px-8 py-5 rounded-full mb-10 backdrop-blur-md transition-all ${isDarkMode ? 'bg-white/[0.03] border-white/10 focus-within:border-indigo-500/50' : 'bg-black/[0.02] border-black/5 focus-within:border-indigo-500/30'}`}>
        ...
        <input 
            ...
            className="flex-1 bg-transparent outline-none w-full font-bold text-xl placeholder:opacity-20"
            value={searchTerm} 
      ...
      <div className="space-y-4">
        {filteredModels.length === 0 ? (
            <div className={`text-center py-12 rounded-3xl border border-dashed ${isDarkMode ? 'border-white/5 text-white/10' : 'border-slate-200 text-slate-400'}`}>
            ...
            filteredModels.map((model) => (
              <div key={model.name} className={`group p-6 rounded-[35px] border flex items-center justify-between transition-all backdrop-blur-md hover:-translate-y-1 ${isDarkMode ? 'bg-white/[0.02] border-white/5 hover:bg-white/[0.05] hover:border-white/10' : 'bg-black/[0.01] border-black/5 hover:bg-black/[0.03] hover:border-black/10'}`}>
                ...
                <div className="flex items-center gap-12">
                  ...
                  <button 
                    onClick={() => { setModelToConfirm(model.name); setShowConfirm(true); }}
                    className="w-14 h-14 flex items-center justify-center bg-red-500/10 text-red-500 hover:bg-red-600 hover:text-white rounded-[20px] transition-all"
                  >
                    <Trash2 size={24} />
                  </button>
                </div>
